...
function App() {
  ...
  const [filters, setFilters] = useState<Filters>({ status: [], country: [], year: [], series: [], type: [], period: [], mint: [] });
  ...
  const loadCoins = useCallback(() => {
    ...
    fetch(`/api/coins?${params}`)
      .then(r => r.json())
      .then(data => { setCoins(Array.isArray(data) ? data : []); setLoading(false); })
  ...
  const clearFilters = () => { setSearch(''); setStatusFilter(''); setCountryFilter(''); setYearFilter(''); };
  const hasActiveFilters = search || statusFilter || countryFilter || yearFilter;

  const groupedCoins = coins.reduce((acc: Record<string, Coin[]>, coin) => {
    const yr = coin.year ? String(coin.year < 0 ? `${Math.abs(coin.year)} BC` : coin.year) : 'Unknown';
    if (!acc[yr]) acc[yr] = [];
    acc[yr].push(coin);
    return acc;
  }, {});
        ...
        <div className="max-w-screen-xl mx-auto px-4 sm:px-6 h-16 flex items-center gap-4">
          <div className="flex items-center gap-3 flex-shrink-0">
            <div className="w-8 h-8 rounded-full bg-gradient-to-br from-amber-300 to-amber-600 flex items-center justify-center shadow-[0_0_15px_rgba(251,191,36,0.3)]">
              <svg className="w-4 h-4 text-black" viewBox="0 0 24 24" fill="currentColor">
                ...
                <circle cx="12" cy="12" r="4" fill="none" stroke="rgba(0,0,0,0.3)" strokeWidth="1.5" />
              </svg>
            </div>
            <span className="font-semibold text-white/90 hidden sm:block tracking-tight">Coin Catalog</span>
          </div>
          ...
          {dbFiles.length > 1 && (
            <select value={selectedFile} onChange={e => setSelectedFile(e.target.value)}
              ...
              {dbFiles.map(f => <option key={f} value={f} className="bg-[#1a1a2e]">{f.replace('.db', '')}</option>)}
            </select>
          )}
          {dbFiles.length === 1 && (
            <span className="text-white/40 text-sm hidden sm:block">{selectedFile.replace('.db', '')}</span>
          )}
        ...
        {sidebarOpen && (
          <aside className="w-56 flex-shrink-0 space-y-4">
            <div className="flex items-center justify-between">
              ...
              {hasActiveFilters && <button onClick={clearFilters} className="text-xs text-amber-400 hover:text-amber-300">Clear all</button>}
            </div>
            {filters.status.length > 0 && (
              <div className="space-y-1">
                <label className="text-[10px] text-slate-500 uppercase tracking-widest block">Status</label>
                <select value={statusFilter} onChange={e => setStatusFilter(e.target.value)}
                  ...
                  {filters.status.filter(Boolean).map(s => <option key={s} value={s} className="bg-[#1a1a2e]">{s}</option>)}
                </select>
              </div>
            )}
            {filters.country.length > 0 && (
              <div className="space-y-1">
                <label className="text-[10px] text-slate-500 uppercase tracking-widest block">Country</label>
                <select value={countryFilter} onChange={e => setCountryFilter(e.target.value)}
                  className="w-full bg-white/5 border border-white/10 text-white/80 text-sm rounded-lg px-3 py-2 focus:outline-none focus:border-amber-400/40">
                  <option value="" className="bg-[#1a1a2e]">All</option>
                  {filters.country.filter(Boolean).map(c => <option key={c} value={c} className="bg-[#1a1a2e]">{c}</option>)}
                </select>
              </div>
            )}
            {filters.year.length > 0 && (
              <div className="space-y-1">
                <label className="text-[10px] text-slate-500 uppercase tracking-widest block">Year</label>
                <select value={yearFilter} onChange={e => setYearFilter(e.target.value)}
                  className="w-full bg-white/5 border border-white/10 text-white/80 text-sm rounded-lg px-3 py-2 focus:outline-none focus:border-amber-400/40">
                  <option value="" className="bg-[#1a1a2e]">All</option>
                  {filters.year.filter(Boolean).map(y => <option key={y} value={y} className="bg-[#1a1a2e]">{y}</option>)}
                </select>
              </div>
            )}
          </aside>
          ...
          {!selectedFile ? (
            <div className="flex flex-col items-center justify-center py-32 gap-4">
              <div className="w-20 h-20 rounded-full bg-white/5 border border-white/10 flex items-center justify-center">
                <svg className="w-10 h-10 text-slate-600" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1">
              ...
              <div className="text-center">
                ...
                <p className="text-slate-500 text-sm mt-1">
                  Place your <code className="text-amber-400 bg-amber-400/10 px-1 rounded">.db</code> files in the <code className="text-amber-400 bg-amber-400/10 px-1 rounded">data/</code> directory
                </p>
              </div>
            </div>
          ) : loading ? (
            ...
            <div className="flex flex-col items-center justify-center py-32 gap-4">
              ...
              {hasActiveFilters && <button onClick={clearFilters} className="text-amber-400 hover:text-amber-300 text-sm underline">Clear filters</button>}
            </div>
          ) : (
              ...
              {Object.entries(groupedCoins).map(([year, yearCoins]) => (
                <section key={year}>
                  <div className="flex items-center gap-3 mb-4">
                    <h2 className="text-amber-400/80 font-mono text-sm font-semibold tracking-wider">{year}</h2>
                    <div className="flex-1 h-px bg-white/5" />
                    <span className="text-slate-600 text-xs font-mono">{yearCoins.length}</span>
                  </div>
                  <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-3">
                    {yearCoins.map(coin => (
