...
class CoinController extends Controller
{
    ...
    {
        $data = $this->sanitizeRichTextFields($this->validated($request));

        try {
            $data = $translator->translateBulgarianFields($data);
        } catch (Throwable $exception) {
            throw ValidationException::withMessages([
                'title.bg' => 'Automatic translation failed: '.$exception->getMessage(),
            ]);
        }
    ...
    {
        $data = $this->sanitizeRichTextFields($this->validated($request, $coin));
    ...
    {
        foreach (['front_description', 'back_description', 'description'] as $field) {
            foreach (array_keys($data[$field] ?? []) as $locale) {
                $value = strip_tags((string) $data[$field][$locale], '<p><br><strong><em><u><s><ol><ul><li><blockquote><h2><h3>');
                $data[$field][$locale] = preg_replace('/<([a-z0-9]+)\s+[^>]*>/i', '<$1>', $value) ?? $value;
            }
        }
