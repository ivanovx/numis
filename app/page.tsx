...
export default function Home() {
  ...
  const [filters, setFilters] = useState<Filters>({ status: [], country: [], year: [], series: [] });
      ...
      <header className="sticky top-0 z-30 bg-[#0a0a14]/90 backdrop-blur-xl border-b border-white/5">
        <div className="max-w-screen-xl mx-auto px-4 sm:px-6 h-16 flex items-center gap-4">
          ...
          {dbFiles.length > 1 && (
            ...
            >
              {dbFiles.map(f => (
                <option key={f} value={f} className="bg-[#1a1a2e]">{f.replace('.db', '')}</option>
              ))}
            </select>
          )}
          {dbFiles.length === 1 && (
        ...
        {sidebarOpen && (
          <aside className="w-56 flex-shrink-0 space-y-4">
            <div className="flex items-center justify-between">
              ...
              {hasActiveFilters && (
                <button onClick={clearFilters} className="text-xs text-amber-400 hover:text-amber-300">Clear all</button>
              )}
            </div>
        ...
        <main className="flex-1 min-w-0">
          {!selectedFile ? (
            ...
            <div className="flex flex-col items-center justify-center py-32 gap-4">
              ...
              {hasActiveFilters && (
                <button onClick={clearFilters} className="text-amber-400 hover:text-amber-300 text-sm underline">Clear filters</button>
              )}
            </div>
          ) : (
            <div className="space-y-10">
              {Object.entries(groupedCoins).map(([year, yearCoins]) => (
                <section key={year}>
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-3">
                    {yearCoins.map(coin => (
                      <CoinCard key={coin.id} coin={coin as any} onClick={() => setSelectedCoinId(coin.id)} />
                    ))}
                  </div>
                </section>
              ))}
