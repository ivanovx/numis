...
class ArtistsGrid extends Component
{
    ...
    {
        if (! in_array($column, ['name', 'slug', 'coins_count'], true)) {
            return;
        }
    ...
    {
        Artist::whereKey($this->selected)->get()->each->delete();

        $this->selected = [];
        $this->resetPage();
        session()->flash('status', 'Selected artists deleted.');
    }
    ...
    public function render(): View
    {
        ...
        return view('livewire.admin.artists-grid', compact('artists'));
    }
}
